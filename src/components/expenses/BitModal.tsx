      ...
      {rows.length === 0 ? (
        ...
        <div className="space-y-2">
          {rows.map((r) => (
            <div
              key={r.id}
              className="grid grid-cols-12 items-center gap-3 rounded-xl border border-sand-200 bg-sand-50/50 px-3 py-2.5"
            >
              <div className="col-span-12 sm:col-span-5">
                <div className="text-sm font-medium text-ink-900 truncate">
                  {r.merchant}
                </div>
                <div className="text-xs text-ink-400">{formatDate(r.date)}</div>
              </div>
