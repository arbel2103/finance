...
export function RangePicker({ value, onChange, monthOptions }: Props) {
  return (
    <div className="flex flex-wrap items-center gap-2">
      <div className="inline-flex rounded-xl bg-sand-100 p-1 gap-1">
        {presets.map((p) => (
          <button
            key={p.id}
      ...
      {value.preset === 'custom' && (
        <div className="flex items-center gap-2 text-sm">
          <select
            ...
            className="rounded-lg border border-sand-200 bg-white px-2 py-1.5 text-sm"
          >
            {monthOptions.map((m) => (
          ...
          <span className="text-ink-400">עד</span>
          <select
            value={value.to}
