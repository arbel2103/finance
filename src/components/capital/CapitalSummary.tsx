...
function TotalCard({
  ...
  return (
    <div className="relative rounded-2xl p-4 border shadow-soft bg-sage-600 border-sage-700 text-white">
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-sage-50">סה״כ הון</span>
        <button
          onClick={() => setOpen((o) => !o)}
          className="text-sage-50/90 hover:text-white"
      ...
      {excluded.length > 0 && (
        <div className="mt-1 text-xs text-sage-50/90">
      ...
      {open && (
        <>
          ...
          <div className="absolute left-2 top-11 z-30 w-56 rounded-xl border border-sand-200 bg-white p-2 text-ink-900 shadow-card">
            <div className="px-2 py-1 text-[11px] text-ink-400">
              מה לכלול בסה״כ הון:
            </div>
            {rows.map((r) => {
              ...
              return (
                <label
                  key={r.key}
                  className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 text-sm hover:bg-sand-50"
                >
                  <input
                    type="checkbox"
                    checked={included}
