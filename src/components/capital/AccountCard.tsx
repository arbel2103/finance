...
export function AccountCard({ account }: { account: Account }) {
  ...
  return (
    <Card className="flex flex-col gap-4">
          ...
          {account.goals.map((g) => {
            ...
            const pct =
              g.targetAmount && g.targetAmount > 0
                ...
                : null
            return (
              <div key={g.id} className="rounded-xl bg-sand-50 px-3 py-2">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="flex items-center gap-2">
                    {g.targetAmount ? (
                      ...
                      <span className="text-ink-400">ללא יעד</span>
                    )}
                    <button
      ...
      {/* חישוב חכם */}
      {goalsWithTarget && (
        <div className="rounded-xl bg-sage-50 px-4 py-3 text-sm text-sage-700">
          {remaining > 0 ? (
            <p className="leading-relaxed">
              ...
              <input
                ...
                className="no-spinner mx-1 w-14 rounded-md border border-sage-200 bg-white px-1.5 py-0.5 text-center text-sage-800"
              />{' '}
              חודשים, צריך להפקיד{' '}
